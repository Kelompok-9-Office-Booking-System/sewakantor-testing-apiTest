...
for(i=0;i<16;i++)
{
	...
	if(i==0)
	{
		...
		rslt = WS.sendRequest(findTestObject('Postman/Admin/Unit/Update'))
	...
	else if(i==1)
	{
		...
		GlobalVariable.admin_UnitID=0
		rslt = WS.sendRequest(findTestObject('Postman/Admin/Unit/Update'))
		
		WS.verifyResponseStatusCode(rslt, 404)
		
		WS.verifyElementPropertyValue(rslt, 'message', 'unit not found')
	...
	else if(i==2)
	{
		...
		WS.verifyElementPropertyValue(rslt, 'message', 'unit updated successfully')
